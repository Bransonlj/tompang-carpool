...
public class CarpoolController {
    ...
    public CarpoolController(CarpoolQueryService carpoolQueryService) {
        this.carpoolQueryService = carpoolQueryService;
    }

    @GetMapping("/{id}")
    public ResponseEntity<Carpool> getCarpoolById(@PathVariable String id) {
        Optional<Carpool> carpool = carpoolQueryService.getCarpoolById(id);
        if (carpool.isEmpty()) {
            throw new ResourceNotFoundException("Carpool not found: " + id);
        }

        return ResponseEntity.ok(carpool.get());
    }
